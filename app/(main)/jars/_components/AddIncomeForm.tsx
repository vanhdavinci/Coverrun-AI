...
const AddIncomeForm = ({ lastIncomeAllocations, onIncomeAdded }: { lastIncomeAllocations: any, onIncomeAdded: () => void }) => {
  const { user } = useUser();
  ...
  const [jarAllocations, setJarAllocations] = useState<{ [key: string]: number }>(() => {
  ...
  const getTotalAllocation = (): number => {
    return Object.values(jarAllocations).reduce((sum: number, value: number) => sum + value, 0);
  };
  ...
  const totalAllocation: number = getTotalAllocation();
  const isValidAllocation: boolean = Math.abs(totalAllocation - 100) < 0.01;

  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className="flex items-center gap-2 bg-blue-600 hover:bg-green-700">
